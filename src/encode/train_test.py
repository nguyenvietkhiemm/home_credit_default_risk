...
def label_encode(train, test):
    ...
    utils.to_feature(train.drop(["data"], axis=1).add_prefix(PREF), name="train")
    utils.to_feature(test.drop(["data"], axis=1).add_prefix(PREF), name="test") 
    
    print("label encode train test")

def label_encode2():
    PREF = "f002_"
    train["data"]=1
    test["data"]=0
    trte = trte[app_categorical_features + ["data"]]
    trte = encode.label_encode(trte, app_categorical_features)
    train = trte[trte["data"]==1]
    test = trte[trte["data"]==0]
    utils.to_feature(train.drop(["data"], axis=1).add_prefix(PREF), name="train")
